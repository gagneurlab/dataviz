...
data <- data.frame(x = x, y_true = y_true, y = y, fitted = fitted_values, residuals = residuals)

# Calculate intercept and slope for the equation display
intercept <- coef(model)[1]
slope <- coef(model)[2]

## Line equations
expectation_equation <- expression(E(y ~ "|" ~ x) == beta[0] + beta[1] * x)
fit_equation <- expression(hat(y) == hat(beta)[0] + hat(beta)[1] * x)

# Prepare labels for the third data point and its residual
data$label <- ifelse(data$x == 3, paste("x[", data$x, "] = ", data$x, ", y[", data$x, "] = ", round(data$y, 2), sep=""), "")
data$residual_label <- ifelse(data$x == 3, paste("hat(epsilon)[", data$x, "]"), "")

## global plot lims
xlims <- c(min(data$x)-0.5, max(data$x)+2)
ylims <- c(min(data$y)-0.5, max(data$y)+1.5)
...
p1 <- ggplot(data, aes(x = x, y = y)) +
  geom_point(aes(y = y_true), color = expectation_color) + # expected points
  geom_abline(intercept = beta_0, slope = beta_1, color = expectation_color, linetype = "solid") + # true expectation line
  geom_segment(
    aes(x=x, xend = x, y=y_true, yend = y),
    arrow = arrow(type = "closed", length = unit(0.2, "cm")),
    color = expectation_color ) + # errors with arrows geom_text(aes(label = label), hjust = 1.5, vjust = 2, color = "darkblue") + # Label the third data point with "xi, yi"
  geom_point(color = data_color) + # Data points 
  geom_text(data = subset(data, x == 3), aes(label = "paste('(', 'x'[i], ',', ' ', 'y'[i], ')')"), parse = TRUE, nudge_x = -0.3, nudge_y = -0.3, color = data_color) +
  annotate("text", x = x[3] -0.5, y = y_true[3] + 0.4, label = "paste('(', 'x'[i], ',', ' ', E(y[i]), ')')", parse = TRUE, color = expectation_color) + # Label fitted points with mathematical notation
  geom_text(aes(x = x[3] + 0.2, y = (y[3] + y_true[3])/2, label = "epsilon[i]"), parse = TRUE,  color = expectation_color) +
  geom_text(aes(x = max(x)+2, y = max(y) + 0.6), label = expectation_equation, parse = TRUE, hjust = 1, color=expectation_color) +
  xlim(xlims) + ylim (ylims) +
  ggtitle("Data generation model") +
  theme(axis.text.x = element_blank(), axis.text.y = element_blank(),
        plot.title = element_text(hjust = 0.5, face = "bold")) # Remove x and y axis ticks
...
p2 <- ggplot(data, aes(x = x, y = y)) +
  geom_point(aes(y = fitted), color = fit_color ) + # Fitted points
  geom_abline(intercept = intercept, slope = slope, color = fit_color , linetype = "solid") + # Fitted line
  geom_segment(
    aes(x=x, xend = x, y=fitted, yend = y),
    arrow = arrow(type = "closed", length = unit(0.2, "cm")),
      color = fit_color  ) + # Residuals with arrows geom_text(aes(label = label), hjust = 1.5, vjust = 2, color = "darkblue") + # Label the third data point with "xi, yi"
  geom_point(color = data_color) + # Original data points 
  geom_text(data = subset(data, x == 3), aes(label = "paste('(', 'x'[i], ',', ' ', 'y'[i], ')')"), parse = TRUE, nudge_x = -0.3, nudge_y = -0.3, color = data_color) +
 annotate("text", x = 3 -0.3, y = fitted_values[3] + 0.5, label = "paste('(', 'x'[i], ',', ' ', hat(y)[i], ')')", parse = TRUE, color = fit_color ) + # Label fitted points with mathematical notation
geom_text(aes(x = x[3]+0.2, y = (y[3] + fitted[3])/2, label = "hat(epsilon)[i]"), parse = TRUE,  color = fit_color ) +
geom_text(aes(x = max(x) + 2, y = max(fitted)+0.5), label = fit_equation, parse = TRUE, hjust = 1, color=fit_color ) +
  xlim(xlims) + ylim (ylims) +
  ggtitle("Least squares fit") +
  theme(axis.text.x = element_blank(), axis.text.y = element_blank(),
        plot.title = element_text(hjust = 0.5, face = "bold")) # Remove x and y axis ticks
...
As for the t-test presented for comparing two groups in Section \@ref(student-t), the degrees of freedom is equal to the number of data points $n$ minus two, the number of model parameters ($\beta_0$ and $\beta_1$). 
